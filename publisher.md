...
Resolved sidecars are then copied into `.agent/skills/<gav-skill-name>/agent-docs.zip`, extracted into `.agent/skills/<gav-skill-name>/` with skill-spec layout (`SKILL.md`, `references/`, `assets/`, `scripts/`), rewritten so extracted `SKILL.md` frontmatter `name` matches the rewritten folder, marked with `.agent-docs`, and surfaced through generated skills under `.agent/skills/...` for local LLM agents.
